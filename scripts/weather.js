    ...
    if (data) {
      ...
      if (weatherData) {
        ...
        if (forecast) {
          ...
          let card = ` <h3> Hourly Forecast </h3><div class="forecast-box">`;
          for (const items of hourlyForecast) {
            card += `<div class="forecast-card"><span> ${new Date(items.dt * 1000).getHours()}:${new Date(items.dt * 1000).getMinutes()} </span><span>${Math.round(kelvintoCelsius(items.main.temp))} &deg;C</span>
                    </div>`;
          }
          card += `</div>`;
